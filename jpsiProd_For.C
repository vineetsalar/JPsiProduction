...
//const Double_t nCC0_2540 = (10.0*1658)/362.0; ;  
//const Double_t nJpsi0 = (0.18*1658)/362.0; ; 

Double_t a0 = 1.0/TMath::Sqrt(mQ*ep0);
Double_t epConMod = (8.0*pi*16.0*16.0/27.0)*(a0/mQ);

//const Double_t nCC0_2540 = (12.0*1658)/362.0; ;  // Initial with shadowing 
//const Double_t nJpsi0_2540 = (0.18*1658)/362.0;  // Initial pp to be used in RAA to be divided by 0.65 // now it is 0.932 (at three places) 

//const Double_t nCC0_2540 = (12.0*1658)/362.0; ;  // Initial with shadowing 
//const Double_t nJpsi0_2540 = (0.122*1658)/362.0;  // Initial pp to be used in RAA to be divided by 0.932 // now it is 0.778 
//const Double_t JPsiIntSh_2540 = 1.0/0.778; // on 20.04.2015


const Double_t nCC0_2540_All = 22.0;
const Double_t Fugicity = 0.45;   


const Double_t nCC0_2540 = (nCC0_2540_All*Fugicity*1658)/362.0; ;  // Initial with shadowing 

const Double_t nJpsi0_2540 = (0.1223*1658)/362.0;  // Initial pp to be used in RAA to be divided by 0.932 // now it is 0.778 
const Double_t JPsiIntSh_2540 = 1.0/0.83; // on 20.04.2015
...
//Double_t a0 = 0.915;
//Double_t epConMod = 4.0*16.0*(16.0*16.0*16.0*pi*a0*a0*a0*ep0)/(6.0*3.0*3.0);

Double_t a0 = 1.0/TMath::Sqrt(mQ*0.64);
Double_t epConMod = 16.0*(8.0*pi*16.0*16.0/27.0)*(a0/mQ);

const Double_t nCC0_2540 = (10.0*1658)/362.0; ;  
const Double_t nJpsi0_2540 = (0.18*1658)/362.0; ; 
//const Double_t FormTau=0.89;
const Double_t FormTau=0.0;
const Double_t CNMSigmaMid=0.15;
const Double_t CNMSigmaFor=0.15; //fm2
...
const Double_t nCC0_2540_All = 0.607;
const Double_t Fugicity = 0.45;   

const Double_t nCC0_2540 = (nCC0_2540_All*Fugicity*1658)/362.0;
const Double_t nJpsi0_2540 = (0.00123*1658)/362.0; 
const Double_t JPsiIntSh_2540 = 1.0/0.9962; // on 20.04.2015
...
const Double_t nCC0_2540 = 0.5;
const Double_t nJpsi0_2540 = 0.01; 
...
//Double_t SigmaFac = 0.5;
Double_t SigmaFac = 1.0; //nominal
//Double_t SigmaFac = 1.5;
...
const Double_t ALICEFac = 0.880881;
const Double_t CMSFac = 1.04585;

const Double_t SS = 5.0*1.5*1600*ALICEFac;
...
Double_t SigmaDMod(Int_t Flag, Double_t q0);
...
Double_t FormRateMC_P_New(Double_t Temp, Int_t i);
...
Double_t RegenratedQuarkoniaNPart(Double_t NCC, Double_t NJPsi, Double_t R0, Double_t IntShadowing);
...
//Double_t CNMVsNPart(Double_t Pt, Double_t NPart);
...
//Double_t TauToTemp_LatticeEOS(Double_t ssCent, Double_t R0Cent, Double_t tau1); 
...
TFile *fileShadowingNPart = new TFile("ShawQuarkonia_NPart.root","R");

TGraph *grShJPsiVsNPart = (TGraph*)fileShadowingNPart->Get("grShJPsiVsNPart");
TGraph *grShJPsiPtCutVsNPart = (TGraph*)fileShadowingNPart->Get("grShJPsiPtCutVsNPart");
TGraph *grUpsilonShNPart = (TGraph*)fileShadowingNPart->Get("grShUpsilonVsNPartMid");
...
TFile *fileShadowingNPart = new TFile("2804_ShawQuarkonia_NPart.root","R");
...
TFile *fileJPsiComover = new TFile("OutJPsiPionCross.root","R");
TGraph *grfJPsiPionCross = (TGraph*)fileJPsiComover->Get("grCalcSigmaJPsiPion");

TFile *fileUpsilonComover = new TFile("OutUpsilonPionCross.root","R");
...
void jpsiProd_For()
{
  ...
  if(QQbarVar==1){sprintf(OutFileName,"For_JPsiCalculations.root");}
  if(QQbarVar==2){sprintf(OutFileName,"For_PsiCalculations.root");}
  if(QQbarVar==3){sprintf(OutFileName,"For_ChiCCalculations.root");}

  if(QQbarVar==4){sprintf(OutFileName,"For_Y1SCalculations.root");}
  if(QQbarVar==5){sprintf(OutFileName,"For_Y2SCalculations.root");}
  if(QQbarVar==6){sprintf(OutFileName,"For_Y3SCalculations.root");}
  if(QQbarVar==7){sprintf(OutFileName,"For_ChiBCalculations.root");}
  ...
  cout<<" checking Value of TAA: "<<TAA(0,4)<<"   "<<TAA(4,8)<<"   "<<TAA(8,12)<<"   "<<TAA(20,40)<<"   "<<endl;


  /*
  //========= Shadowing from Ramona Calculations ==================//
  cout<<"JPsi Raa vs pT : "<< HistJpsiRaaShVsPt->Integral()/HistJpsiRaaShVsPt->GetBinWidth(0)<<endl;
  cout<<"Upsilon Raa vs pT mid: "<< HistUpsilonRaaShVsPtMid->Integral()/HistUpsilonRaaShVsPtMid->GetBinWidth(0)<<endl;
  new TCanvas;
  HistJpsiRaaShVsPt->Draw();
  new TCanvas;
  HistUpsilonRaaShVsPtMid->Draw();
  */
  ...
  Double_t sigDMod[2000];
  Double_t sigFMod[2000];
  ...
  for(int i=0; i<NS; i++) {
    ...
    sigD[i] =SigmaGluonDissS(ss[i])*hbarc2*10.0;
    sigF[i] = SigmaFS(ss[i])*hbarc2*10;
    
    sigDMod[i]=SigmaGluonDissSMod(ss[i])*hbarc2*10.0;
    sigFMod[i]=SigmaFSMod(ss[i])*hbarc2*10.0;
  ...
  grSigD->GetYaxis()->SetTitle("#sigma_{D} (mb) ");
  ...
  grSigF->SetLineColor(8);
  grSigF->GetXaxis()->SetTitle("s (GeV^{2})");
  grSigF->GetYaxis()->SetTitle("#sigma_{F} (fm^{2}) ");


  
  TGraph *grSigDMod = new TGraph(NS, ss, sigDMod);
  grSigDMod->SetName("grSigDMod");
  grSigDMod->SetTitle("grSigDMod");
  grSigDMod->SetLineWidth(2);
  grSigDMod->SetLineColor(2);
  grSigDMod->GetXaxis()->SetTitle("s (GeV^{2})");
  grSigDMod->GetYaxis()->SetTitle("#sigma_{D} (Mod) (mb) ");


  TGraph *grSigFMod = new TGraph(NS, ss, sigFMod);
  grSigFMod->SetName("grSigFMod");
  grSigFMod->SetTitle("grSigFMod");
  grSigFMod->SetLineWidth(2);
  grSigFMod->SetLineColor(8);
  grSigFMod->GetXaxis()->SetTitle("s (GeV^{2})");
  grSigFMod->GetYaxis()->SetTitle("#sigma_{F} (mb) ");
  ...
  //if(isUps ==0){cout<<" Value of Int CNM for JPsi : ======== > "<<CNMInt(0.25)<<"   "<<CNMInt(6.25)<<endl;}

  //if(isUps ==1 || isUps ==2){cout<<" Value of Int CNM for Upsilon: ======== > "<<CNMInt(0.25)<<"   "<<CNMInt(6.25)<<endl;}
  ...
  for(int i = 0; i<=NTempD; i++) {
    ...
    LambdaD_GluonDissVac[i]= DissRateXu(3.6,TempD[i])/hbarc;
  ...
  //====================== Dissociation rates vs Temp graphs =====================//
  ...
  TGraph *DissRate_KharAll = new TGraph(NTempD,TempD,LambdaD_KharAll);
  ...
  TGraph *DissRate_GluonDissVac = new TGraph(NTempD,TempD,LambdaD_GluonDissVac);
  ...
  new TCanvas;
  DissRate_GluonDissVac->Draw("APL");
  DissRate_Khar36->Draw("Lsame");
  ...
  for(int i = 0; i<=NPtD; i++) {
    ...
    DissRatePionVsPt1[i]=PionJPsiDiss(PtD[i], 0.100)/hbarc;

    //cout<<PtD[i]<<"  "<< DissRateKharVsPt1[i]<<"   "<<DissRateKharVsPt2[i]<<"   "<<DissRateKharVsPt3[i]<<endl;
  }
  ...
  TGraph *grDissRatePionVsPt1 = new TGraph(NPtD,PtD,DissRatePionVsPt1);
  ...
  //===========================================================================================================================//
	...
	if(FORM ==1){
	  LambdaFMC_P1[i]=FormRateMC_P(TempF[i],2.25);
	  LambdaFMC_P2[i]=FormRateMC_P(TempF[i],3.25);
	  LambdaFMC_P3[i]=FormRateMC_P(TempF[i],4.25);
	}
      }
      
      // MC Int 
      TGraph *grFormRateR = new TGraph(NTempF,TempF,LambdaFR);
      grFormRateR->SetLineWidth(2);
      grFormRateR->SetLineColor(2);
      
      //MC Pt =0
      TGraph *grFormRateR0 = new TGraph(NTempF,TempF,LambdaFR0);
      grFormRateR0->SetLineWidth(2);
      grFormRateR0->SetLineColor(1);
      
      //Triple Integral
      TGraph *grFormRate_TripleInt = new TGraph(NTempF,TempF,LambdaF);
      grFormRate_TripleInt->SetMarkerStyle(20);
      grFormRate_TripleInt->SetLineWidth(2);
      grFormRate_TripleInt->SetLineColor(4);
      
      TGraph *grFormRate_TripleInt_P = new TGraph(NTempF,TempF,LambdaF_TripleInt);
      ...
      cout<<" ==================== Formation Rate vs Pt ===================== " <<endl;
  ...
  //Double_t PtALICE[30]={0.25,0.50,0.75,1.0,1.25,1.50,1.75,2.0,2.25,2.50,2.75,3.0,3.25,3.50,3.75,4.25,4.75,5.25,5.75,6.25,6.75,7.25,7.75,8.25,8.75,9.25,9.75,10.25};
  ...
  Double_t NCCMB=nCC0_2540*NColl(0,40)/NColl(0,2);
  Double_t NJPsiMB=nJpsi0_2540*NColl(0,40)/NColl(0,2);

  Double_t tauCMB=Tau0ToTauC(2,T0MB,R0MB);
  Double_t tauHMB=TauCToTauh(2,tauCMB,R0MB);
  Double_t tauFMB=TauhToTauf(2,tauHMB,R0MB);

  CalculateTandf(3, T0MB, R0MB, tauCMB, tauHMB,tauFMB,ssMB);
  cout<<"NTau: "<<NTau<<"  ssMB "<<ssMB<<endl;

  //for(int i=0;i<=NTau;i++){
    //cout<<" Temp : "<<TempTau[i]<<" FQGP  "<< fQGP[i]<<endl;
  //}
  
  for(int i=0;i<200;i++)
    {
      //cout<<" Resetting the histograms in pT loop : "<<HistRegenJpsiPt[i]->GetName()<<endl;
  ...
  for(int i=0; i<NNPtALICE; i++) {
    ...
    ShadowDissALICEPt[i]=HistJpsiRaaShVsPt_Y2440->Interpolate(PtALICE[i]);
    ...
    //Double_t JPsiIntSh_2540 = 1.0/0.932;
   
    FormationALICEPt[i]= NJPsiRegenALICEPt[i]/(NJPsiMB*JPsiIntSh_2540*NJPsi0ALICEPt[i]);
  ...
  for(int i=0; i<NNPtCMS; i++) {
    ...
    FormationCMSPt[i]=NJPsiRegenCMSPt[i]/(NJPsiMB*JPsiIntSh_2540*NJPsi0CMSPt[i]);

    cout<<" Formation Parts: "<<IntFormVsPt(PtCMS[i],R0MB)<<"  "<<NJPsiRegenCMSPt[i]<<"  "<<endl;
    ...
    cout<<ShadowDissCMSPt[i]<<"   "<<GluonDissCMSPt[i]<<"   "<<PionDissCMSPt[i]<<"   "<<ShadowDissCMSPt[i]*PionDissCMSPt[i]*GluonDissCMSPt[i]<<
      "  "<<FormationCMSPt[i]<<"  "<<RAACMSPt[i]<<endl;
  ...
  Double_t PionDissRap =  PionDiss_PtInt(6.25);
    ...
    for(int i=0; i<NN; i++) {
      ...
      Double_t nJpsi = nJpsi0_2540*nColl/nColl0;
      Double_t nCC = nCC0_2540*nColl/nColl0;
      
      Double_t ssCent = ss05*(funTwoComp->Eval(nPart[i])/dNdEtabyNpartby2[0]);  
      ...
      cout<<" calculating the Temp Vs Tau in Cent Loop"<<endl;

      CalculateTandf(3, T0Cent, R0Cent, tauCCent, tauHCent,tauFCent,ssCent);
	...
	//cout<<" Temp : "<<TempTau[j]<<" FQGP  "<< fQGP[j]<<endl;
      ...
      if(isUps == 1|| isUps == 2 ){cout<<" save Time =============================== "<<endl;}
      

      for(int j=0;j<200;j++)
	{
	  //cout<<" Resetting the histograms in Cent loop : "<<HistRegenJpsiPt[j]->GetName()<<endl;
	...
	for(int j=0;j<=NTau;j++){
      ...
      //using forward shadowing because finally it will be comapred to ALICE data
      if(isUps==1){intCNMEffectCMS = CNMVsNPart(nPart[i],grShUpsilonVsNPartFor);}
      if(isUps==2){intCNMEffectCMS = CNMVsNPart(nPart[i],grShUpsilonVsNPartFor);}
	...
	//CNMEffectsALICE[i] =CNMVsNPart(0.25, nPart[i]);
	CNMEffectsALICE[i] =CNMVsNPart(nPart[i],grShJPsiVsNPart_Y2440);
	PionDissALICE[i]= PionDiss_PtInt(0.25);
	
	intdissALICE = IntDiss_PtInt(0.25,nPart[i]);
	...
	intformALICE = RegenratedQuarkoniaNPart(nCC,nJpsi,R0Cent,JPsiIntSh_2540);
	cout<<" Regenrating Quarkonia : "<<intformALICE<<endl;
      }
    ...
    legd8->AddEntry(grCNMALICE_M,"CNM Effects","L");
    ...
    if(isUps==1){  
      ...
      legd10->AddEntry(grCNMCMS_M,"CNM Effects","L");
      legd10->AddEntry(grPionDissCMS_M,"Comover","L");
      legd10->AddEntry(grFormCMS_M,"Formation","L");
      legd10->AddEntry(grRAACMS_M,"Total (R_{AA})","L");
...
Double_t DissFuncXu(Double_t pT, Double_t T, Double_t n)
{
  ...
  for(int i=0;i <NNq;i++){
    ...
    Double_t Func = q*SigmaDMod(1,q)*fac;
    //SigmaDMod(Int_t Flag, Double_t q0)
...
Double_t RegenratedQuarkoniaNPart(Double_t NCC, Double_t NJPsi, Double_t R0, Double_t IntShadowing) 
{
  ...
  for(int i=0; i<NN; i++) {
    ...
    RegenQuarkDeno = RegenQuarkDeno + ( NJPsi* IntShadowing * NJPsi0ALICEPt);
  

    //cout<< GluonDissALICEPt <<"   "<<
...
Double_t SigmaQuasiGluon(Int_t Flag , Double_t P1, Double_t T)
{
  ...
  //double mTSquare = mJpsi*mJpsi +P1*P1;
  
  //P1 = TMath::Sqrt(P1*P1 + mTSquare*TMath::SinH(2)*TMath::SinH(2)); 
 
  Double_t E1 = TMath::Sqrt( P1*P1 + m1Square);

  //Double_t P2Step = 0.002;
...
//================================================================== Modified corss sections ======================================//
//Double_t epConMod = (16.0*16.0*16.0*pi*a0*a0*a0*ep0)/(6.0*3.0*3.0);
...
Double_t SigmaDMod(Int_t Flag, Double_t q0)
{
  ...
  if( (QQbarVar == 2 || QQbarVar == 5) && q0 > ep0 ) {SigmaDq0 = epConMod*pow( (q0/ep0 - 1.0), 1.5) * pow( (q0/ep0 - 3.0), 2.0)  /pow((q0/ep0),7.0);}  
...
Double_t SigmaGluonDissSMod(Double_t s)
{
  ...
  //Double_t SigmaFac = 1.0;

  return SigmaFac*SigmaDq0;
}
...
Double_t SigmaFSMod(Double_t s)
{
  ...
  SigmaF = SigmaFac*SigmaD*48.0*kJpsi2/(36.0*kcc2);
...
Double_t Shadowing(Double_t SigmaAbs, Double_t NPart)
{
  ...
  Double_t L= 1.5*RPb*TMath::Sqrt(NPart/416.0);
  //cout<<"L : "<<L<<endl;
...
/*
Double_t CNMVsNPart(Double_t PtMin, Double_t NPart)
{
  //PtMin=0.25; ALICE
  //PtMin=6.25; CMS
  //Double_t Ptmax=0;
  ...
  if(PtMin==0.25 && isUps ==0){yy=grShJPsiVsNPart->Eval(NPart);}  
  if(PtMin==6.25 && isUps ==0){yy=grShJPsiPtCutVsNPart->Eval(NPart);}  
 
  if(PtMin==0.25 && isUps ==1){yy=grUpsilonShNPart->Eval(NPart);}
  if(PtMin==0.25 && isUps ==2){yy=grUpsilonShNPart->Eval(NPart);}
...
/*
Double_t CNMInt(Double_t PtMin)
{
  ...
  for(int i=0;i<=NN_Pt;i++) {
    ...
    if(isUps==0){ShadowingFactor = HistJpsiRaaShVsPt->Interpolate(Pt);}
    if(isUps==1 || isUps==2){ShadowingFactor = HistUpsilonRaaShVsPtMid->Interpolate(Pt);}
  
    sum= sum + (ShadowingFactor*yield);
    sumPt = sumPt+yield;  
  }
  return sum/sumPt;
}
