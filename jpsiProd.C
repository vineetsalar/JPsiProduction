...
const Double_t nCC0_21_All = 22.0; 
const Double_t Fugicity = 0.45;   

const Double_t nCC0_21 = (nCC0_21_All*Fugicity*1658)/362.0; ;  // Initial with shadowing 
const Double_t nJpsi0_21 = (0.1223*1658)/362.0;  // Initial pp to be used in RAA to be divided by 0.65 // now it is 0.932 (at three places) 
...
//const Double_t nCC0_21 = 0.5;
//const Double_t nJpsi0_21 = 0.01; 
...
Double_t RegenratedQuarkoniaNPart(Double_t NCC, Double_t NJPsi, Double_t R0, Double_t IntSh);
...
TFile *fileUpsilon2SComover = new TFile("OutUpsilon2SPionCross.root","R");
...
void jpsiProd()
{
  ...
  */
  ...
  Double_t NCCMB=nCC0_21*NColl(0,40)/NColl(0,2);
  Double_t NJPsiMB=nJpsi0_21*NColl(0,40)/NColl(0,2);
  ...
  for(int i=0; i<NNPtALICE; i++) {
    ...
    //Double_t JPsiIntSh = 1.0/0.932;
   
    FormationALICEPt[i]= NJPsiRegenALICEPt[i]/(NJPsiMB*JPsiIntSh_1*NJPsi0ALICEPt[i]);
  ...
  for(int i=0; i<NNPtCMS; i++) {
    ...
    FormationCMSPt[i]=NJPsiRegenCMSPt[i]/(NJPsiMB*JPsiIntSh_21*NJPsi0CMSPt[i]);
  ...
  Double_t PionDissRap =  PionDiss_PtInt(6.25);
	...
	CNMEffectsALICE[i] =CNMVsNPart(nPart[i],grShJPsiVsNPart_Y1);
	...
	intformALICE = RegenratedQuarkoniaNPart(nCC,nJpsi,R0Cent,JPsiIntSh_21);
...
Double_t CNMVsNPart(Double_t PtMin, Double_t NPart)
{
  ...
  //TGraph *grShJPsiVsNPart = (TGraph*)fileShadowingNPart->Get("grShJPsiVsNPart");
  //TGraph *grShJPsiPtCutVsNPart = (TGraph*)fileShadowingNPart->Get("grShJPsiPtCutVsNPart");
  //TGraph *grUpsilonShNPart = (TGraph*)fileShadowingNPart->Get("UpsilonShNPart");
  //for(int i=0;i<=NN_Pt;i++) {
  //Double_t Pt=PtMin+i*Ptstep;
  //if(isUps==0){yield=Jpsi_Pt->GetBinContent(Jpsi_Pt->FindBin(Pt));}
  //if(isUps==1){yield=Y1S_Pt->GetBinContent(Y1S_Pt->FindBin(Pt));}
