...
const Double_t CNMSigmaMid=0.25;
const Double_t CNMSigmaFor=0.35;
//fm2
...
Double_t IntDiss_All(Double_t T0Cent, Double_t R0Cent, Double_t taufCent, Double_t tauhCent, Double_t PtMin);
Double_t IntDiss_PtInt(Double_t T0Cent, Double_t R0Cent, Double_t taufCent, Double_t tauhCent, Double_t PtMin);
Double_t IntDiss_PtIntVsPt(Double_t T0Cent, Double_t R0Cent, Double_t tauCCent, Double_t tauhCent, Double_t PtMin, Double_t PtMax);
...
void Oct2013Store_jpsiProd()
{
 ...
 // Npart vs Ncoll graphs =========================================
  ...
  //cout<<" tau : "<<"  Long "<<"  Cylnd "<<endl;

  cout<<" Tau "<< " TauC "<<" TauH "<<endl;
  
  
  Double_t TauC1=Tau0ToTauC(1,T0*TMath::Power((funTwoComp->Eval(Npart(0,2)))/dNdEtabyNpartby2[0],1.0/3.0),R05);
  Double_t TauH1 = TauCToTauh(1,Tau0ToTauC(2,T0*TMath::Power((funTwoComp->Eval(Npart(0,2)))/dNdEtabyNpartby2[0],1.0/3.0),R05),R05);
  Double_t TauC2=Tau0ToTauC(2,T0*TMath::Power((funTwoComp->Eval(Npart(0,2)))/dNdEtabyNpartby2[0],1.0/3.0),R05);
  Double_t TauH2 = TauCToTauh(2,Tau0ToTauC(2,T0*TMath::Power((funTwoComp->Eval(Npart(0,2)))/dNdEtabyNpartby2[0],1.0/3.0),R05),R05);
  
  for(int i=0;i<TauN;i++)
    {
      TauA[i]=TauAMin+TauStep*i;
      TempA[i]=TauToTemp_All(1,T0*TMath::Power((funTwoComp->Eval(Npart(0,2)))/dNdEtabyNpartby2[0],1.0/3.0),R05,TauA[i]);      
      TempB[i]=TauToTemp_All(2,T0*TMath::Power((funTwoComp->Eval(Npart(0,2)))/dNdEtabyNpartby2[0],1.0/3.0),R05,TauA[i]);      
      
      //FQGP =================
      FQGP1[i]=FQGP(1,R05,TauC1,TauH1,TauA[i]);
      FQGP2[i]=FQGP(2,R05, TauC2,TauH2,TauA[i]);
    }
  ...
  TGraph *grFQGPVsTau = new TGraph(TauN,TauA,FQGP1);
  grFQGPVsTau->SetLineColor(2);
  grFQGPVsTau->GetYaxis()->SetTitle("FQGP");
  grFQGPVsTau->GetXaxis()->SetTitle("#tau (fm)");

  TGraph *grFQGPVsTau2 = new TGraph(TauN,TauA,FQGP2);
  grFQGPVsTau2->SetLineColor(1);
  ...
  cout<<" NPart "<<"   "<<" T0 "<<"     R0Cent  "<<"   Tauf "<< "    Tauh "<<"  TauhLong  "<<endl;
  ...
  for(int i=0;i<12;i++){
    ...
    for(int j=0;j<TauN;j++){
      ...
      TempA1[i][j]=TauToTemp_All(2,T0TempVsTau[i],R0Cent,TauA[j]); 
    }
  }
  ...
  if( CheckFunctions ==1){
    ...
    for(int l = 0; l<=NTemp; l++) {
      ...
      //Dissociation Rates
      if(Disso ==1){
	LambdaD_Khar[l]= 197*DissRateXu(0.0, Temp[l]); // Xu and Kharjeev as a function of pT
	LambdaD_Khar36[l]= 197*DissRateXu(3.6, Temp[l]); // Xu and Kharjeev as a function of pT
	LambdaD_Khar86[l]= 197*DissRateXu(8.6, Temp[l]); // Xu and Kharjeev as a function of pT

	LambdaD_KharAll[l]= DissRateIntpT(Temp[l]); // Xu and Kharjeev int pT
	...
	if(Disso ==1){
	  ...
	  DissRateKharVsPt1[l]= DissRateXu(P[l],0.200);
	  DissRateKharVsPt2[l]= DissRateXu(P[l],0.400);
	  DissRateKharVsPt3[l]= DissRateXu(P[l],0.600);
	}
	if(Form ==1){
    ...
    // Diss rates
    TLegend *legd1 = new TLegend( 0.24,0.67,0.69,0.86);
  ...
  if( CheckFunctions ==0 || CheckFunctions ==1){
    ...
    Double_t CNMEffectsCMS[20];
    Double_t CNMEffectsALICE[20];
    ...
    int NNPt =16.0;
    Double_t RAACMSPt[20];
    Double_t Pt[20]={0.25,0.75,1.25,1.75,2.25,2.75,3.25,3.75,4.25,4.75,5.25,5.75,6.25,6.75,7.25,7.75,8.25,8.75};
	...
	//cout<<Npart(0,40)<<endl;
	//cout<<"T0MB: "<<T0MB<<" R0MB "<<R0MB<<"   "<<tauCMB<<"  "<<tauHMB<<endl;


	RAACMSPt[i]=IntDiss_PtIntVsPt(T0MB,R0MB,tauCMB,tauHMB,Pt[i],Pt[i+1]);

	cout<<Pt[i]<<"  "<<RAACMSPt[i]<<endl;
}


    // RAA graphs
    TGraph *grRAACMSPt_M = new TGraph(NNPt,Pt,RAACMSPt);
    ...
    //TGraphErrors *grRaaPtALICEFor = new TGraphErrors(nbinsALICE, NPartALICE, RaaALICE, ErrNPartALICE, SystErrALICE);  
    TGraphErrors *grRaaPtALICEFor = new TGraphErrors(nbinsPtALICEFor, PtALICEFor, RaaPtALICEFor, RaaPtStatErrALICEFor, RaaPtStatErrALICEFor);  
    ...
    */
    ...
    for(int i=0; i<NN; i++) {
      ...
      //Jpsi Pt integrated 
      Double_t intdissCMS = IntDiss_PtInt(T0Cent,R0Cent,tauCCent,tauHCent,6.25);
        
      //Upsilon Pt integrated 
      //Double_t intdissCMS = IntDiss_PtInt(T0Cent,R0Cent,tauCCent,tauHCent,0.25);
      ...
      RAACMS[i] = nJpsiFCMS[i]*CNMEffectsCMS[i];
     
      Double_t intdissALICE=0; 
      Double_t intformALICE=0;
      if(isUps==0){
	CNMEffectsALICE[i] =Shadowing(CNMSigmaFor,nPart[i]);
	intdissALICE = IntDiss_PtInt(T0Cent,R0Cent,tauCCent,tauHCent,0.25);
	intformALICE =NJpsiStatALICE[i];
      ...
      nJpsiFALICE[i] = ( (jpsiDissALICE[i]*CNMEffectsALICE[i]) + jpsiFormALICE[i] );
      

      RAAALICE[i] = nJpsiFALICE[i];
      
      cout << nPart[i]<<"  "<<"    " << T0Cent <<"     "<<tauCCent <<"  "<<R0Cent<<"  "<<RAACMS[i]<<"   "<<RAAALICE[i]<<endl;
    }
...
Double_t TauToTemp_All(Int_t Flag, Double_t T0Cent, Double_t R0Cent, Double_t Tau) 
{
  ...
  if( Flag ==1) 
    {
      ...
      if(Tau >= tau0 && Tau <= TauC) {Temp = pow(T0Cent*T0Cent*T0Cent*tau0/Tau, 1.0/3.0);}
      if(Tau > TauC && Tau <= TauH ){Temp = TC;}
      //if( Tau > TauH ){Temp = }
    }
  ...
  if(Flag ==2) {
    ...
    if(Tau >= tau0 && Tau <= TauC) { Temp = pow(Sum, 1.0/3.0);}
    if(Tau > TauC && Tau <= TauH ){Temp = TC;}
...
Double_t FQGP(Int_t Flag, Double_t R0Cent, Double_t TauCCent, Double_t TauHCent, Double_t Tau)
{
  ...
  if(Flag ==1){
    Nomi=aq*(Tau-TauCCent);
    Deno=(aq-ah)*Tau;
    fqgp = 1 - (Nomi/Deno); 
  }
 
  if(Flag ==2) { 
    Nomi=aq*TauCCent*(R0Cent+0.5*aT*TauCCent*TauCCent)*(R0Cent+0.5*aT*TauCCent*TauCCent) - ah*Tau*(R0Cent+0.5*aT*Tau*Tau)*(R0Cent+0.5*aT*Tau*Tau);
    Deno= (aq - ah)*Tau*(R0Cent+0.5*aT*Tau*Tau)*(R0Cent+0.5*aT*Tau*Tau);
    fqgp = Nomi/Deno;
  }

  if(Tau<= TauCCent){fqgp =1.0;}
  if(Tau > TauHCent){fqgp =0.0;}
...
Double_t IntDiss()
{
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    sumd = sumd + DissRateIntpT(T);
  }
  return exp(-sumd*step);
}
...
Double_t IntDiss(Double_t T0Cent, Double_t R0Cent, Double_t taufCent, Double_t PtMin)
{
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    sumd = sumd +  theta*DissRateXu(PtMin,T);
  }
  return exp(-sumd*step);
}
...
Double_t IntDiss_All(Double_t T0Cent, Double_t R0Cent, Double_t tauCCent, Double_t tauhCent, Double_t PtMin)
{

  Double_t step = 0.1;
  int N1 = (int)((tauhCent-tau0)/step);
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    Double_t theta =1.0;
    if(tau < FormTau * (PtMin/mJpsi) ) theta =0;
    Double_t T = TauToTemp_All(2,T0Cent,R0Cent,tau);
    Double_t fQGP =FQGP(2,R0Cent,tauCCent, tauhCent,tau);
    sumd = sumd +  fQGP*theta*DissRateXu(PtMin,T);
    //sumd = sumd +  fQGP*theta*1.0;
...
Double_t IntDiss_PtInt(Double_t T0Cent, Double_t R0Cent, Double_t tauCCent, Double_t tauhCent, Double_t PtMin)
{
  ...
  for(int i=0;i<=NN_Pt;i++) {
    ...
    sum= sum + (yield*IntDiss_All(T0Cent,R0Cent,tauCCent,tauhCent,Pt) );
    sumPt= sumPt+ yield;  
  }
  return sum/sumPt;
}
