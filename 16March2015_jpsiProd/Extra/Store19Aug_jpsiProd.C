...
void jpsiProd()
{
  ...
  cout<<Jpsi_Pt->GetMean()<<"   "<<Jpsi_Pt->GetRMS()<<"  "<<Jpsi_Pt->GetSize()<<"  "<<Jpsi_Pt->FindBin(6.5)<<endl;
  Double_t PtCut=6.5;
  
  JPsiPtCut=new TH1D("JPsiPtCut","JPsiPtCut",500,0.0,50.0);
  
  cout<<" Bin no "<<"  "<<" Bin Centre "<<" Bin Content "<<endl;

  for(int i = Jpsi_Pt->FindBin(PtCut);i<Jpsi_Pt->GetSize();i++)
    {	
      
      JPsiPtCut->SetBinContent(i,Jpsi_Pt->GetBinContent(i));
      
      cout<<i<<"   "<<JPsiPtCut->GetBinCenter(i)<< "    "<<JPsiPtCut->GetBinContent(i)<<endl;
  ...
  gPad->SetTicks();
  gPad->SetLogy();
  Jpsi_Pt->SetLineWidth(2);
  Jpsi_Pt->SetLineColor(2);
  Jpsi_Pt->GetYaxis()->SetTitleOffset(1.4);
  Jpsi_Pt->GetXaxis()->SetTitle("p_{T} (GeV/c)");
  Jpsi_Pt->GetYaxis()->SetTitle("dN/dp_{T}");
  Jpsi_Pt->Draw();
  gPad->SaveAs("JPsiPt.pdf");
  gPad->SaveAs("JPsiPt.png");
  
  
  new TCanvas;
  gPad->SetTicks();
  gPad->SetLogy();
  JPsiPtCut->Scale(1.0/JPsiPtCut->Integral());
  JPsiPtCut->Draw();
  //Jpsi_Pt->Draw("same");
  
  cout<<JPsiPtCut->GetMean()<<"   "<<JPsiPtCut->GetRMS()<<"  "<<JPsiPtCut->Integral()<<endl;
  ...
  TF1 *funDnDetaNPart = new TF1("funDnDetaNPart"," ( [0]*TMath::Power(x,[1]) + [2]*TMath::Power(x,[3]) )/(0.5*x)",10.0,400);
  ...
  cout<<" NPart "<<"   "<<" T0 "<<" R0Cent  "<<"  Tauf "<<endl;
  for(int i=0;i<12;i++)
    {
      ...
      cout<< NPartTempVsTau[i] <<"  "<< T0TempVsTau[i] <<"  "<<R0Cent1<<"   "<<T0ToTauf(1,T0TempVsTau[i],R0Cent1)<<endl;
      //cout<<" Tauf: "<<T0ToTauf(1,T0TempVsTau[i],R0Cent1)<<"   "<<T0ToTauf(2,T0TempVsTau[i],R0Cent1)<<endl;;
      TaufA[i]=T0ToTauf(1,T0TempVsTau[i],R0Cent1);
    }
  ...
  if( CheckFunctions ==0 || CheckFunctions ==1){
    ...
    for(int i=0; i<NN; i++) {
      ...
      Double_t intdissCMS = IntDiss_PtInt(T0Cent,R0Cent,taufCent,6.5);
      ...
      Double_t intformALICE=0.0;
      jpsiDissALICE[i] = intdissALICE;
      jpsiFormALICE[i] = intdissALICE * intformALICE * nCC*nCC/(nJpsi);
      nJpsiFALICE[i] = ( jpsiDissALICE[i] + jpsiFormALICE[i] );
      RAAALICE[i] = nJpsiFALICE[i]*CNMEffects[i];
      
      cout << nPart[i]<<"  "<<"    " << T0Cent <<"     "<<taufCent <<"  "<<R0Cent<<"  "<<RAACMS[i]<<"   "<<RAAALICE[i]<<endl;
   ...
   legd7->Draw("same");
    ...
    tb->DrawLatex(0.22,0.16,"J/#psi #rightarrow #mu^{+} #mu^{-}");
    ...
    legd8->AddEntry(grRaaALICE,"ALICE","P");
    legd8->AddEntry(grDissALICE_M,"Dissociation","L");
    ...
    tb->DrawLatex(0.21,0.24,"#varUpsilon #rightarrow #mu^{+} #mu^{-}");
    c1->Update();
    // gPad->Update();
...
Double_t IntDiss(Double_t T0Cent, Double_t R0Cent, Double_t taufCent, Double_t PtMin)
{
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    //JPsi
    if(tau < FormTau* (6.5/mJpsi) ) theta =0;
    //Upsilon 
    //if(tau < FormTau * (1.0/mJpsi) ) theta =0;
...
Double_t IntDiss_PtInt(Double_t T0Cent, Double_t R0Cent, Double_t taufCent, Double_t PtMin)
{
  ...
  for(int i=1;i<=NN_Pt;i++) {
    ...
    //cout<<Pt<<"    "<<Jpsi_Pt->GetBinContent(JPsiPtCut->FindBin(Pt))<<endl;

    Double_t yield=JPsiPtCut->GetBinContent(JPsiPtCut->FindBin(Pt));
    

    sum= sum + ( yield*IntDiss(T0Cent,R0Cent,taufCent,Pt) );
    //sum= sum + IntDiss(T0Cent,R0Cent,taufCent,Pt) ;
