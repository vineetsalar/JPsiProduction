 ...
 //  return;
  ...
  if( CheckFunctions ==1){
    ...
    for(int l = 0; l<=NTemp; l++) {
      ...
      //Dissociation Rates
      if(Disso ==1){
	LambdaD_Khar[l]= DissRateXu(0.0, Temp[l]); // Xu and Kharjeev as a function of pT
	LambdaD_Khar36[l]= DissRateXu(3.6, Temp[l]); // Xu and Kharjeev as a function of pT
	LambdaD_Khar86[l]= DissRateXu(8.6, Temp[l]); // Xu and Kharjeev as a function of pT
	
	LambdaD_GluonDissVac[l]=SigmaQuasiGluon(3,0.0,Temp[l])/197.327;
    ...
    DissRate_GluonDissVac->Draw("PLsame");
    legd1->Draw("same");
...
Double_t IntDiss_All(Double_t T0Cent, Double_t R0Cent, Double_t tauCCent, Double_t tauhCent, Double_t PtMin)
{
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    sumd = sumd +  fQGP*theta*(SigmaQuasiGluon(1,PtMin,T)/197.237);
