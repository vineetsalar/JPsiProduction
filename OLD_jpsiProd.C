...
const Double_t nCC0 = (10.0*1658)/362.0; ;  // Initial with shadowing 
const Double_t nJpsi0 = (0.18*1658)/362.0;  // Initial pp to be used in RAA to be divided by 0.65 
...
void jpsiProd()
{
  ...
  //HistRegenJpsiPt[1000];
  //dataGen[i]= new RooDataSet(Form("dataGen%d",i),"weighted GenWdataset",varlist);
  //new TCanvas;
  //Jpsi_Pt->Draw();
  ...
  Double_t ssCent = ss05*(funTwoComp->Eval(Npart(0,2))/dNdEtabyNpartby2[0]);  
  ...
  CalculateTandf(1, T0Cen, R05, TauC1, TauH1,TauF1,ssCent);
  ...
  CalculateTandf(2,T0Cen,R05,TauC2,TauH2,TauF2,ssCent);
  NTauC= NTau;
  ...
  NTau=0;
  //  CalculateTandf_LatticeEOS(ssCent,R05);
  CalculateTandf(3,T0Cen,R05,TauC2,TauH2,TauF2,ssCent);

 
  cout<<" NTau "<<NTau<<" ssCent "<<ssCent<<endl;
  ...
  grTempVsTauAnaC->SetLineColor(6);
  grTempVsTauAnaC->SetLineWidth(2);
  grTempVsTauAnaC->SetLineStyle(6);
  ...
  grFQGPVsTauAnaC->SetLineColor(6);
  grFQGPVsTauAnaC->SetLineWidth(2);
  grFQGPVsTauAnaC->SetLineStyle(6);
  ...
  legd5->AddEntry(grTempVsTauAnaC,"Lattice EOS","L");
...
/*
  for(int i=0;i<=NTau;i++)
    {
      sprintf(NameHistG,"HistRegenJpsiPt_%d",i);
      //cout<<" Temp "<<TempTau[i]<<" FQGP "<<fQGP[i]<<endl;
      HistRegenJpsiPt[i] = new TH1D(NameHistG,NameHistG,100,0.0,50.0);
      FormRateMC_P_New(TempTau[i],i);  
      cout<<i<<"  "<<NTau<<" Get Name  "<<HistRegenJpsiPt[i]->GetBinContent(HistRegenJpsiPt[i]->FindBin(10))<<endl;
    }
    new TCanvas;
    HistRegenJpsiPt[10]->Draw();
  */
  ...
  //for(int j=0;j<=100;j++){cout<<" Bin Center "<<HistRegenJpsiPt[0]->GetBinCenter(j)<<endl;}
  ...
  //Double_t nPart[100]={Npart(0,4),Npart(4,16),Npart(16,40)};
    ...
    for(int i=0; i<NN; i++) {
      ...
      CalculateTandf(2, T0Cent, R0Cent, tauCCent, tauHCent,tauFCent,ssCent);
    
      /*
      for(int j=0;j<=NTau;j++)
	{
	  sprintf(NameHistG,"HistRegenJpsiPt_Cent_%d",j);
	  //cout<<" Temp "<<TempTau[i]<<" FQGP "<<fQGP[i]<<endl;
	  HistRegenJpsiPt[j] = new TH1D(NameHistG,NameHistG,100,0.0,50.0);
	  FormRateMC_P_New(TempTau[j],j);  
	  cout<<j<<"  "<<NTau<<" Get Name  "<<HistRegenJpsiPt[j]->GetBinContent(HistRegenJpsiPt[j]->FindBin(10))<<endl;
	}
      new TCanvas;
      HistRegenJpsiPt[10]->Draw();
      */
	...
	//Branching ratio can be multiplied inside function also
...
Double_t FormRateMC_P_New(Double_t Temp, Int_t iTau) 
{
  ...
  //cout<<" Temp inside function "<<Temp<<" xx "<<xx<<endl;
  //char HistName[1000];
  //sprintf(HistName,Form("HistPt_%f"),xx);
  //sprintf(HistName,"HistPt_%f",xx);
  //TH1D *Hist = new TH1D(HistName,HistName,100.0,0.0,50.0);
  // cout<<" HistName inside loop "<<HistName<<"   "<<Hist->GetName()<<endl;
