...
//Parameters
Double_t mUpsilon = 9.46;
Double_t mD = 5.280;
Double_t mQ = 5.280;
Double_t mQ2 = mQ*mQ;
Double_t ep0=2.0*mQ-mUpsilon;
...
const Double_t T0 = 0.640; 
const Double_t Tf = 0.160;

const Double_t tauf = pow(T0/Tf, 3.)*tau0;
const Double_t R0 =0.92*7.11;
const Double_t nCC0 = 0.50;
const Double_t nUpsilon0 = 0.01; 
...
Double_t Shadowing(Double_t NPart);
...
void UpsilonProd()
{
  ...
  TFile *OutFile = new TFile("UpsilonProd.root","RECREATE");
  ...
  //============== Upsilon gluon dissociation cross section graph =======================//
  Double_t smin=0.0;
  Double_t smax = 60.0;
  Double_t step = 0.2;
  ...
  TGraph *grdNDetaNpart = new TGraph(10,NPartdNdEta,dNdEtabyNpartby2);
  ...
  TF1 *funPol3 = new TF1("funPol3","pol3",0.0,500);
  funPol3->SetLineColor(1);

  lcat[3] = new TLegend( 0.20,0.76,0.70,0.86);
  ...
  new TCanvas;
  grdNDetaNpart->Fit("funDnDetaNPart","M","",5.3,400.0);
  grdNDetaNpart->Fit("funPol3","M","",5.3,400.0);
  grdNDetaNpart->Draw("AP");
  funPol3->Draw("same");
  ...
  grTempVsNpart->SetMarkerColor(2);
  ...
  cout<<"NPartVal: "<<"   "<<" NCollVal: "<<endl;
  for(int i=0;i<40;i++)
    {
      NPartVal[i]=Npart(i,i+1);
      NCollVal[i]=NColl(i,i+1);
      cout<<NPartVal[i]<<"   "<<NCollVal[i]<<endl;
    }
  ...
  Double_t TauAMax= 10;
  Double_t TauAMin=0.2;
  int TauN= (TauAMax - TauAMin )/0.1;
  
  for(int i=0;i<TauN;i++)
    {
      TauA[i]=TauAMin+0.1*i;
      TempA[i]=TauToTemp(1,TauA[i]);      
      TempB[i]=TauToTemp(2,TauA[i]);      
    }
  ...
  for(int i=0;i<12;i++)
    {
      ...
      Double_t R0Cent1 = R0*TMath::Power(NPartTempVsTau[i]/Npart(0,2),0.5);
      cout<<" T0 for differnt Cent: "<<T0TempVsTau[i]<<endl;
      cout<<" Tauf: "<<T0ToTauf(1,T0TempVsTau[i],R0Cent1)<<"   "<<T0ToTauf(2,T0TempVsTau[i],R0Cent1)<<endl;;
      TaufA[i]=T0ToTauf(2,T0TempVsTau[i],R0Cent1);
    }
 
  for(int i=0;i<12;i++){
    for(int j=0;j<TauN;j++){
      Double_t R0Cent = R0*TMath::Power(NPartTempVsTau[i]/Npart(0,2),0.5);
  ...
  cout<<"NPart:   "<<"          "<<"Tauf: "<<endl;
  for(int i=0;i<40;i++)
    {
      cout<<Npart(i,i+1)<<"     "<<fa3->Eval(Npart(i,i+1))<<endl;
  ...
  if( CheckFunctions ==1){
    ...
    new TCanvas;
    gPad->SetTicks();
    grDissRateVsPt1->GetYaxis()->SetRangeUser(0.0,0.12);
  ...
  if( CheckFunctions ==0){
    ...
    Double_t nPart[100]={Npart(0,2),Npart(2,4),Npart(4,6),Npart(6,8),Npart(8,10),Npart(10,12),
      ...
      Double_t jpsiForm[100], jpsiDiss[100], nUpsilonF[100], RAA[100];
      cout << "nPart "  << "NColl "<<"  Temp "<< "           Tau  " <<"      Disso  "<<"     Form "<<endl;
      
      NN=12;  
      
      for(int i=0; i<NN; i++) {
	//nPart[i] = nPartMin + nstep * i;
      
	Double_t nColl = fa2->Eval(nPart[i]);  // ??? 
	Double_t nUpsilon = nUpsilon0*nColl/nColl0;
	...
	Double_t R0Cent = R0*TMath::Power(nPart[i]/Npart(0,2),0.5);          
     	Double_t intdiss = IntDiss(T0Cent,R0Cent,taufCent,5.56);
      
	//Double_t intform = IntForm(T0Cent,R0Cent,taufCent);
	
	//Double_t intdiss=1.0;
	Double_t intform=0.0;
	
	//Double_t intform = 0.0;
	
	//jpsiDiss[i] = intdiss * nUpsilon / nUpsilon;
	//jpsiForm[i] = intdiss * intform * nCC*nCC/(pi*RCent*RCent)/nUpsilon;
	
	jpsiDiss[i] = intdiss;
      //jpsiForm[i] = intdiss * intform * nCC*nCC/(pi*RCent*RCent*nUpsilon);
	
      //to make it fast
	jpsiForm[i] = intdiss * intform * nCC*nCC/(nUpsilon);
	nUpsilonF[i] = ( jpsiDiss[i] + jpsiForm[i] );
	RAA[i] = nUpsilonF[i];
	cout << nPart[i]<<"  "<<nColl<< "    " << T0Cent <<"     "<< taufCent <<"  "<<jpsiDiss[i]<<"   "<<jpsiForm[i]<<endl;
	cout<<nUpsilon<<"  "<<nCC<<endl;
    }
    
    // RAA graphs
    TGraph *grRAA = new TGraph(NN,nPart,RAA);
    grRAA->SetLineWidth(2);
    grRAA->GetXaxis()->SetTitle("N_{Part}");
    grRAA->GetYaxis()->SetTitle("R_{AA} ");
    
    TGraph *grForm = new TGraph(NN,nPart,jpsiForm);
    grForm->SetLineWidth(2);
    grForm->SetLineColor(2);
    
    TGraph *grDiss = new TGraph(NN,nPart,jpsiDiss);
    grDiss->SetLineWidth(2);
    grDiss->SetLineColor(4);
...
Double_t TauToTemp(Int_t Flag, Double_t Tau) 
{
  ...
  if(Flag ==2) {
    Double_t aT = 0.1;
    Double_t z0=1.8*tau0;
    Double_t vZ=1.4;    
    Double_t Deno = (R0+0.5*aT*Tau*Tau)*(R0+0.5*aT*Tau*Tau)*(z0+vZ*Tau);
    Double_t Num = T0*T0*T0*(R0+0.5*aT*tau0*tau0)*(R0+0.5*aT*tau0*tau0)*(z0+vZ*tau0);
...
Double_t T0ToTauf(Int_t Flag, Double_t T0Cent, Double_t R0Cent) 
{
  ...
  //Double_t Tf=0.170;
  TaufValLong = T0Cent*T0Cent*T0Cent*tau0/(Tf*Tf*Tf);
  ...
  if(Flag ==2) {
    ...
    do{
      ...
      TaufValCylnd = TaufG -  ( (Tf*Tf*Tf*( (0.25*vZ*pow(aT,2)*pow(TaufG,5))
					    +(0.25*z0*pow(aT,2)*pow(TaufG,4))
					    +(R0Cent*aT*vZ*pow(TaufG,3))
					    +(z0*R0Cent*aT*pow(TaufG,2))
					    +(pow(R0Cent,2)*vZ*TaufG)
					    +(z0*R0Cent*R0Cent)) -C)/( Tf*Tf*Tf*( (1.25*vZ*pow(aT,2)*pow(TaufG,4))
...
Double_t DissRateXu(Double_t pT, Double_t T)
{
  Double_t mT=TMath::Sqrt(pT*pT+mUpsilon*mUpsilon);
  //Double_t lambdag =16.0;
  Double_t fac=2.0*pi*mUpsilon*mUpsilon/mT;
...
Double_t SigmaD(Int_t Flag, Double_t q0)
{
  ...
  if(Flag ==1 || Flag ==2 ) {SigmaDq0 = epCon*pow(q0-ep0,1.5)/pow(q0,5.0);}  
  return SigmaDq0;
...
Double_t DissRateZhang(Double_t T)
{
  Double_t alphaUpsilon = mUpsilon/T;
  Double_t alphaC = mQ/T;

  Double_t z0=alphaUpsilon;
  if(alphaUpsilon<2.0*alphaC) z0 = 2.0*alphaC; 


  Double_t alphaUpsilon2 = alphaUpsilon*alphaUpsilon;

  Double_t deno = 8.0*alphaUpsilon2*TMath::BesselK(2,alphaUpsilon);
  ...
  for(int i=0; i<NZ; i++) {
    ...
    Double_t fun = pow(z2-alphaUpsilon2, 2)*TMath::BesselK(1,z)*SigmaDS(z2*T*T);
    sum = sum+fun;
  }
...
Double_t SigmaDS(Double_t s)
{
  ...
  Double_t q0 = (s-mUpsilon*mUpsilon)/(2.0*mUpsilon); 
...
Double_t SigmaFS(Double_t s)
{
  ...
  Double_t kUpsilon2 = pow(s-mUpsilon*mUpsilon, 2.0);
  Double_t kcc2 = s*(s-4.0*mQ2);

  Double_t SigmaD = epCon*pow(q0-ep0, 1.5)/pow(q0,5.0);  

  SigmaF = SigmaD*48.0*kUpsilon2/(36.0*kcc2);
...
Double_t FormFun(Double_t p1, Double_t p2, Double_t theta1, Double_t theta2, Double_t phi1, Double_t phi2, Double_t T)
{
  ...
  //  ccbar to Upsilon cross section
  //  Double_t FJPsi = 0;
...
Double_t IntDiss(Double_t T0Cent, Double_t R0Cent, Double_t taufCent, Double_t PtMin)
{
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    Double_t Rhog = 2*8*1.202*pow(T, 3.0)/pi2;
    //sumd = sumd +  DissRateIntpT(T)*Rhog;
    sumd = sumd +  DissRateXu(PtMin,T)*Rhog;
  }
  return exp(-sumd*step/hbarc3);
}
...
Double_t IntForm( Double_t T0Cent, Double_t R0Cent, Double_t taufCent)
{
  ...
  for (int i= 0; i<=N1; i++) {
    ...
    Double_t T = TauToTemp(2,T0Cent,R0Cent,tau);
    Double_t Rhog = 2*8*1.202*pow(T, 3.0)/pi2;
    sumd = sumd +  DissRateXu(8.6,T)*Rhog;
    IntDrate[i] =  exp(-sumd*step/hbarc3); 
  }
