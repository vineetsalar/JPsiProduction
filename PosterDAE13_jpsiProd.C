...
Double_t IntDiss_PtIntVsPt(Double_t PtMin, Double_t PtMax);
...
Double_t CalculateTandf(int Flag, Double_t T0Cent, Double_t R0Cent, Double_t tauCCent, Double_t tauhCent);
...
void jpsiProd()
{
  ...
  CalculateTandf(1, T0Cen, R05, TauC1, TauH1);
  NTauL= NTau;
  cout<<" Long " << T0Cen<<"  "<<R05<<"  "<<TauC1<<"  "<<TauH1<<endl;
  ...
  CalculateTandf(2, T0Cen, R05, TauC2, TauH2);

  cout<<" Cylind  " << T0Cen<<"  "<<R05<<"  "<<TauC2<<"  "<<TauH2<<endl;
  ...
  TGraph *grTempVsTauAnaB = new TGraph(NTau,Tau,TempTau);
  ...
  Xaxis3->SetLimits(0.,20.0);

  //TAxis *Yaxis1 = grTempVsTauAna->GetYaxis();
  //Yaxis1->SetLimits(0.,0.50);
  grTempVsTauAna->GetYaxis()->SetRangeUser(0.1,0.45);
  ...
  TGraph *grFQGPVsTau2 = new TGraph(NTau,Tau,fQGP);
  ...
  grDissRateVsPt1->GetYaxis()->SetTitle("#Gamma[p_{J/#psi},T] (fm^{-1})");
  ...
  CalculateTandf(2, T0MB, R0MB, tauCMB, tauHMB);
  
  for(int i=0; i<NNPt; i++) {
    RAACMSPt[i]=IntDiss_All(Pt[i]);
    //RAACMSPt[i]=IntDiss_PtIntVsPt(Pt[i],Pt[i+1]);
  ...
  */
  ...
  for(int i=0; i<NN; i++) {
    ...
    CalculateTandf(2, T0Cent, R0Cent, tauCCent, tauHCent);
    
    //Jpsi Pt integrated 
    Double_t intdissCMS = IntDiss_PtInt(6.25);
...
Double_t IntFormFun2_P(Double_t P, Double_t T)
{
  // cos(theta) integration put p2 = p-p1
  Double_t P1Step = 0.05;
  Double_t P1Start = 0.0;
  Double_t P1End = 10.0;
...
Double_t IntDiss_All(Double_t PtMin)
{
  ...
  for (int i= 0; i<=NTau; i++) {
    ...
    //sumd = sumd +  theta*fQGP*DissRateXu(PtMin,T);
    sumd = sumd +  fQGP[i]*theta*SigmaQuasiGluon(1,PtMin,TempTau[i]);
    //sumd = sumd +  theta*(SigmaQuasiGluon(1,PtMin,T)/197.237);
